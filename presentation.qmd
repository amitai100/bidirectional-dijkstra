...
::: {.incremental}
- A* often outperforms Dijkstra in practice by using a heuristic to focus the search
:::

```{mermaid}
graph LR
    S((Start)) --> A --> B --> C --> T(((Target)))
...
::: {.notes}
A natural question is: what about A-star? It's often faster in practice because it uses a heuristic — an estimate of the remaining distance — to guide the search toward the target. So how can bidirectional Dijkstra be optimal if A-star seems to beat it?
:::

---

## Advice vs. Blind Search

::: {.incremental}
- A* requires **additional advice**: heuristic distance estimates that must be provided externally
- Bidirectional search is optimal among algorithms that operate **without** such advice
:::

::: {.notes}
The key distinction is the information model. A-star gets extra information — the heuristic function — which is not part of the graph itself. It's external advice. The result in this paper is about algorithms that explore the graph blindly, with no additional guidance. In that setting, bidirectional Dijkstra is the best you can do.
:::
...
- Weights induce a distance function $d : V \times V \to \mathbb{R}_{\ge 0}$ (may differ from $d(v,u)$ in directed graphs)
...
- Then $A$ must skip some edge $uv$ where $u$ is closed by Dijkstra, i.e., $d(s,u) < d(s,t)$
- $A$ never queried $uv$ — it doesn't know what's on the other end
:::

::: {.notes}
Now for the lower bound. Suppose some algorithm A uses fewer queries. Then there must be an edge u-v that Dijkstra would have explored but A did not. Crucially, u is a vertex close to s — its distance from s is strictly less than d(s,t). Since A never queried this edge, it has no idea where it leads.
:::
...
- $P$ cannot use $e_2$, otherwise: 
  ...
  - and there would be a st-Path of length:
              $$d(s, v_2) + d(v_2, t) \le d(s, u_1) + d(v_2, t) < d(s,t)$$
...
:::{.callout-important icon="false" .fragment .fade-in}
## Theorem 4
...
::: {.callout-tip icon="false" .fragment .fade-in}
### Claim
$$d_{G'}(s,u_1) \le d_{G}(s,u_1)$$
$$d_{G'}(v_2,t) \leq d_{G}(v_2,t)$$
:::
...
- We will show that the query complexity of the biderctional BFS is $O(\Delta \min\{|E_s|, |E_t|\})$.

---

## Complexity Analysis - Bidirectional BFS

::: {.callout-note icon="false" .fragment .fade-in}
### Lemma
...
- **Remark.** The theorem can be generalized to weighted graphs where the allowed weights come from a set $W$ with $\min(W) > 0$.
...
::: {.callout-warning icon="false" .fragment .fade-in}
...
- Denote by $Q_i$ the sequence of edges queried by $A$ on input $G_i$, before it first queries the $i$-th edge in the last level.
$$\underbrace{e_a e_b \dots e_c}_{Q_i} e_i e_d \dots e_f$$
- Let $k = \operatorname{argmax}_{i} |Q_i|$.

## Proof of Thm. 5 - Lower Bound
- For all $G_i \in \mathcal{X}^*$, denote by $Q^i_k$ the prefix of $Q_k$ before the $i$-th edge in the last level is first queried.
$$Q_k = \underbrace{e_a e_b \dots e_c}_{Q_k^i} e_i e_d \dots e_f \cancel{e_k}$$
- Since $A$ is deterministic, $Q^i_k = Q_i$.
    - That is, $Q_i$'s are distinct prefixes of $Q_k$.

---

## Proof of Thm. 5 - Lower Bound
- Hence, the expected query complexity of $A$ is at least
$$\begin{aligned}
\frac{1}{|\mathcal{X}|}\sum_{i \in \mathcal{X}^*} |Q_i| &\ge \frac{1}{|\mathcal{X}|}\sum_{i=1}^{|\mathcal{X}^*|} i \\
&= \frac{1}{|\mathcal{X}|}\Omega(|\mathcal{X}^*|^2) \\
&= \Omega(|\mathcal{X}|) = \Omega(\Delta^D)
\end{aligned}$$
...
- Apply BFS to explore the vertices from $s$ and $t$ until the until the penultimate layer.
